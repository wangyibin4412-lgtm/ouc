...
/* write serial thread -------------------------------------------------------*/
    ...
    static serial_t *openserial(const char *path, int mode, char *msg)
    {
        ...
        if (!(serial = (serial_t *)malloc(sizeof(serial_t))))
            return NULL;
...
/* get socket error ----------------------------------------------------------*/
    ...
    static int gethostbyname_safe(char *hostname, struct in_addr *addr)
    {
    ...
    static int gentcp(tcp_t *tcp, int type, char *msg)
    {
        ...
        { /* client socket */
            if (!gethostbyname_safe(tcp->saddr, &addr))
            ...
            memcpy(&tcp->addr.sin_addr, &addr, sizeof(addr));
    ...
    static int reqntrip_s(ntrip_t *ntrip, char *msg)
    {
        ...
        if (writetcpcli(ntrip->tcp, (unsigned char *)buff, p - buff, msg) != p - buff)
            return 0;
...
/* ftp thread ----------------------------------------------------------------*/
...
#endif
    {
        ...
        char cmd[2048], env[1024] = "", opt[1024], *proxyopt = "";
        const char *proto;
        int ret;
    ...
    /* open/create canfd ---------------------------------------------------------------*/
    static canfd_t *opencanfd(const char *path, char *msg)
    {
#ifdef _OS_LINUX_
        canfd_t *canfd = NULL;
        int socket_fd;

        canfd = (canfd_t *)malloc(sizeof(canfd_t));
        memset(canfd, 0, sizeof(canfd_t));

        if ((socket_fd = socket(PF_CAN, SOCK_RAW | SOCK_NONBLOCK, CAN_RAW)) < 0)
        {
            trace(1, "Error while opening socket");
            free(canfd);
            return NULL;
        }
        trace(1, "opencanfd,fd=%d\n", socket_fd);
        strcpy(canfd->ifr.ifr_name, path);
        ioctl(socket_fd, SIOCGIFINDEX, &canfd->ifr);
        canfd->addr.can_family = AF_CAN;
        canfd->addr.can_ifindex = canfd->ifr.ifr_ifindex;
        trace(1, "%s at index %d\n", path, canfd->ifr.ifr_ifindex);
        if (bind(socket_fd, (struct sockaddr *)&canfd->addr, sizeof(canfd->addr)) < 0)
        {
            trace(1, "Error in socket bind\n");
            free(canfd);
            return NULL;
        }
        canfd->can_id = 0;
        canfd->socket_fd = socket_fd;
        trace(1, "success to open %s\n", path);
        return canfd;
#else
        return NULL;
#endif
    ...
    /* close canfd ---------------------------------------------------------------*/
    static void closecanfd(canfd_t *canfd)
    {
        if (!canfd)
        {
            return;
...
#ifndef WIN32
        close(canfd->socket_fd);
#endif
        free(canfd);
        canfd = NULL;
    }
#ifndef WIN32
    extern int readcanfd_special(const canfd_t *canfd, struct can_frame *frame_recv)
    {
        int stat = 0;
        int nbytes = 0;
        int nr;
        ...
        nbytes = read(canfd->socket_fd, frame_recv, sizeof(struct can_frame));
        if (nbytes > 0)
        {
            nr = sizeof(frame_recv->data) / sizeof(frame_recv->data[0]);
            if (nr > 0 && frame_recv->can_id == 0x07E8)
            {
                trace(1, "readcanfd,Recv:ID=0x%X DLC=%d Data ", frame_recv->can_id, frame_recv->can_dlc);
                for (int i = 0; i < nr; i++)
                {
                    trace(1, "%X ", frame_recv->data[i]);
                }
                trace(1, "\n");
            }
...
#endif
    /* read canfd ---------------------------------------------------------------*/
    static int readcanfd(canfd_t *canfd, unsigned char *buff, int n, char *msg)
    {
        int nr = 0;
        ...
        if (nbytes > 0)
        {
            ...
            if (nr > 0)
            {
                memcpy(buff, frame_recv.data, sizeof(unsigned char) * nr);
                canfd->can_id = frame_recv.can_id;
                trace(1, "readcanfd,Recv:ID=0x%X DLC=%d Data ", frame_recv.can_id, frame_recv.can_dlc);
                for (int i = 0; i < nr; i++)
                {
                    trace(1, "%02X ", buff[i]);
    ...
    /* write canfd ---------------------------------------------------------------*/
    static int writecanfd(canfd_t *canfd, unsigned char *buff, int n, char *msg)
    {
        int nw = 0;
#ifdef _OS_LINUX_
        int i;
        struct can_frame frame_send;
        if (!canfd || n <= 0 || canfd->can_id == 0)
        {
            trace(1, "writecanfd,return early,n=%d,id=%d!\n", n, canfd->can_id);
            return 0;
        }
        else if (n > 8)
        {
            trace(1, "writecanfd,large n=%d\n", n);
        }
        frame_send.can_id = canfd->can_id;
        frame_send.can_dlc = n;
        for (i = 0; i < n; i++)
        {
            frame_send.data[i] = buff[i];
        }
        nw = write(canfd->socket_fd, &frame_send, sizeof(struct can_frame)); // 发送

        trace(1, "writecanfd,write=%d bytes\n", nw);
        if (nw == -1)
        {
            trace(1, "error in writecanfd,errno=%d\n", errno);
        }
        else
        {
            trace(1, "canid:%02X\n", frame_send.can_id);
    ...
    /* get canfd ----------------------------------------------------------*/
    static int statecanfd(canfd_t *canfd)
    {
        return !canfd ? 0 : (canfd->socket_fd < 0 ? -1 : 2);
    }
    ...
    extern int stropen(stream_t *stream, int type, int mode, const char *path)
    {
        ...
        case STR_HTTP:
            ...
            break;
        case STR_CANFD:
            stream->port = opencanfd(path, stream->msg);
            break;
        default:
            stream->state = 0;
            return 1;
    ...
    extern void strclose(stream_t *stream)
    {
        ...
        if (stream->port)
        {
            ...
            case STR_HTTP:
                ...
                break;
            case STR_CANFD:
                closecanfd((canfd_t *)stream->port);
            }
        }
        else
        {
    ...
    extern int strread(stream_t *stream, unsigned char *buff, int n)
    {
        ...
        case STR_HTTP:
            ...
            break;
        case STR_CANFD:
            nr = readcanfd((canfd_t *)stream->port, buff, n, msg);
            break;
        default:
            strunlock(stream);
            return 0;
    ...
    extern int strwrite(stream_t *stream, unsigned char *buff, int n)
    {
        ...
        case STR_HTTP:
        case STR_CANFD:
            ns = writecanfd((canfd_t *)stream->port, buff, n, msg);
        default:
            strunlock(stream);
            return 0;
        }
    ...
    extern int strstat(stream_t *stream, char *msg)
    {
        ...
        case STR_HTTP:
            ...
            break;
        case STR_CANFD:
            state = statecanfd((canfd_t *)stream->port);
            break;
        default:
            strunlock(stream);
            return 0;
    ...
    /* set bitrate ---------------------------------------------------------------
     * set bitrate to stream for serial
     * args   : stream_t *stream I   stream
     *          int   brate      I   brate
     * return : none
     *-----------------------------------------------------------------------------*/
    extern int setcbrate(stream_t *str, int brate)
    {
        ...
        if (str->type != STR_SERIAL)
            return 0;

        if (!(p = strchr(str->path, ':')))
        {
            sprintf(str->path + strlen(str->path), ":%d", brate);
        }
        else
        {
            if ((q = strchr(p + 1, ':')))
        ...
        strclose(str);
        return stropen(str, STR_SERIAL, STR_MODE_RW, str->path);
    }
